...
function checkPasswordValidity(hash){
    ...
    var suposeOccurence = ranges[0];
    var totalLenght  = ranges[1];
    var occurence = getOccurrence( getKey(hash), password);

    if (occurence >= suposeOccurence){
        if (occurence <= totalLenght){
            return true;
        }
    }
    return false;
...
function getOccurrence(key, password){
    var occurence = password.match(new RegExp(key, 'g'));
    if (occurence) return occurence.length;
}
