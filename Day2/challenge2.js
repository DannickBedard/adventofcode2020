var data = `15-16 f: ffffffffffffffhf
6-8 b: bbbnvbbb
...
const dataSorted = dataToArray(remove_linebreaks(data));

var result = 0;

dataSorted.forEach(hash => {
    if (checkPasswordValidity(hash)){
        result++;
    }
});
...
function checkPasswordValidity(hash){
    var ranges = getRange(hash);
    var password = getPassword(hash);
    var firstPosition = ranges[0];
    var secondPosition  = ranges[1];

    return getOccurrence(getKey(hash), password, firstPosition, secondPosition);
}

function getOccurrence(key, password, firstPosition, secondPosition){
    
    if (password.charAt(firstPosition)  === key){
        if (password.charAt(secondPosition ) != key) return true;
    }
     else {
        if (password.charAt(secondPosition ) === key) return true;
    }
...
function getRange(hash){
    return hash.match(/^\d+|\d+\b|\d+(?=\w)/g).map(function (v) {return +v;});
}


function getKey(hash) {
    return hash.charAt(hash.indexOf(':')-1);
}
